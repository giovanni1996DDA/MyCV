...
const StudyCard = ({study}) => {
  return (
    <div className='group relative grid pb-1 transition-all sm:grid-cols-8 sm:gap-8 md:gap-4 lg:hover:!opacity-100 lg:group-hover/list:opacity-50'>
        ...
        <header className='z-10 mb-2 mt-1 text-xs font-semibold uppercase tracking-wide text-slate-600 sm:col-span-2'>{study.year}</header>
        <div className='z-10 sm:col-span-6'>
            <h3 className='font-medium leading-snug'>
                <div>
                    
                    <span className='inline-flex items-baseline font-medium leading-tight text-cyan-400 hover:text-cyan-500 group/link text-base'>{study.title}</span>
                </div>
            </h3>
            <h4 className='mt-2 text-sm leading-normal'>
            ...
            <ul className='mt-2 flex flex-wrap'>
                {
                    study.technologies.map((technologie) => (
                        <li key={technologie} className='mr-1.5 mt-2'>
